...
function safeXmlFileName(userTitle) {
  const t = (userTitle && String(userTitle).trim()) || 'partido';
  const s = t.replace(/[/\\?%*:|"<>#]/g, '-').replace(/\s+/g, ' ').trim().slice(0, 100);
  return `${s || 'partido'}-${Date.now()}.xml`;
}
...
/**
 * Sube un XML; devuelve id de Drive.
 */
async function uploadXml(drive, folderId, userTitle, xml) {
  const name = safeXmlFileName(userTitle);
  ...
  if (!created.data.id) {
    return { error: 'Drive no devolvió id' };
  }
  return { ok: true, id: created.data.id, title: (created.data.name || name).replace(/\.xml$/i, '') };
}
