...
function aggregateFromXmlString(xml) {
  ...
  for (const g of byKey.values()) {
    ...
    const fgPts = g.fgPoints || 0;
    const totPts = g.pointsTotal != null ? g.pointsTotal : fgPts;
    const points2pt = 2 * (g.fgm2 || 0);
    const points3pt = 3 * (g.fgm3 || 0);
    const pointsFt = Math.max(0, totPts - fgPts);
    ...
    players.push({
      ...
      fga2: g.fga2 || 0,
      fga3: g.fga3 || 0,
      /** Aciertos de campo: 1 conteo por canasta 2P / 3P (2+1/3+1: canasta + TL por separado) */
      fgm2: g.fgm2 || 0,
      fgm3: g.fgm3 || 0,
      /** Puntos por fuente (otros cálculos); en «Anatomía» usar fgm2/fgm3/ftm */
      points2pt,
      points3pt,
      pointsFt,
...
/**
 * Combina varios resultados de aggregateFromXmlString sumando conteos por jugador (mismo equipo + code).
 * PPS, eFG% y TS% salen del acumulado global (no es la media aritmética de partidos).
 * @param {Array<{ players: Array }>} payloads
 * @returns {{ players: Array, meanPps: number|null, hasPlaymaking: boolean }}
 */
function mergeAggregatedPayloads(payloads) {
  const byKey = new Map();
  for (const payload of payloads) {
    for (const p of payload.players || []) {
      ...
      g.ast += p.ast || 0;
      g.tov += p.tov || 0;
      g.fieldGoalPoints += p.fieldGoalPoints != null ? p.fieldGoalPoints : 0;
      g.pointsTotal += p.pointsTotal != null ? p.pointsTotal : (p.fieldGoalPoints || 0);
    }
  }
  ...
  for (const g of byKey.values()) {
    ...
    const fgPts = g.fieldGoalPoints || 0;
    const totPts = g.pointsTotal != null ? g.pointsTotal : fgPts;
    const points2pt = 2 * (g.fgm2 || 0);
    const points3pt = 3 * (g.fgm3 || 0);
    const pointsFt = Math.max(0, totPts - fgPts);
    const pps = g.fga > 0 ? fgPts / g.fga : 0;
    const dPlay = playDenom(g);
    const efgPct = efgFromRows(g);
    const rowTs = { ...g, fgPoints: fgPts, pointsTotal: totPts };
    const tsPct = g.fga + 0.44 * (g.fta || 0) > 0 ? tsFromRows(rowTs) : null;
    const astPct = dPlay > 0 ? round3(100 * (g.ast || 0) / dPlay) : null;
    const tovPct = dPlay > 0 ? round3(100 * (g.tov || 0) / dPlay) : null;
  ...
  const hasPlaymaking = players.some(
    (p) => (p.ast > 0 || p.tov > 0) && playDenom({ fga: p.fga, fta: p.fta, ast: p.ast, tov: p.tov }) > 0
  );
...
module.exports = { aggregateFromXmlString, mergeAggregatedPayloads };
